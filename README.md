...
- **Q:**  我看到配置项中需要输入密码，这安全吗？<br />
  **A:**  密码仅用于登陆苹果账号，用于下单的时候将订单绑定在你的 Apple ID上。
  
- **Q:**  我不相信你，我可以不填密码吗？<br />
  **A:**  是的，你完全可以不填密码。在不填密码的情况下也可以正常使用，订单会已访客账号下单。

- **Q:**  你的插件安全吗？<br />
  **A:**  插件所有代码开源可查，并且网络请求可以在 devTools 查看。当然你的疑虑是正常的，请不要过分相信互联网上的任何人。如果你的安装包并非来自这里，请更加谨慎对待。

- **Q:**  我不相信你，我可以不填密码吗？<br />
  **A:**  是的，你完全可以不填密码。在不填密码的情况下也可以正常使用，订单会已访客账号下单。

- **Q:**  我在我的Chrome中安装插件报错，为什么？<br />
  **A:**  本插件使用 manifest v3.0, 请安装最新版 Chrome 或者 Edge，最少也要 Chrome 88 以上的版本。尽量更新到最新版。

- **Q:**  为什么我看页面没有任何反应，它真的在抢购吗？<br />
  **A:**  是的，代码是在后台静默调用接口，可以按 F12，打开 devTools，控制台里面的 log 信息，并且也可以查看网络请求调用。
...
- **Q:**  我可以同时抢购多部 iPhone 吗？<br />
  **A:**  由于浏览器的插件在一个配置中是共享的，所以一个个人配置的浏览器只能抢购一部。

- **Q:**  那我真的想多部 iPhone 同时捡漏可以吗？<br />
  **A:**  也是可以的，请开多个 Chrome 个人配置，每个配置单独安装插件，他们直接互不影响。

- **Q:**  这样会被封 IP 吗？<br />
  **A:**  如果你一次开多个配置浏览器同时抢购，是可能被封 IP 的，但插件有做处理，当接口不肯访问时，会增加步频时间，直到接口再次可以被访问。

- **Q:**  这样会被封苹果账号吗？<br />
  **A:**  我无法明确回答这个问题。但如果你不放心，可以填写不常用的邮箱地址，并且不要填写密码，这样就以访客模式下单，不会对你的账号造成任何影响。

- **Q:**  我只要配置好就可以了吗？<br />
  **A:**  差不多，但需要注意的是，请保存电脑不要休眠。插件其实只是替代你来职守，电脑需要保持常开状态。

- **Q:**  如果半夜抢到了，但我睡着了怎么办？<br />
  **A:**  目前插件新增语音提醒功能，请在配置中自行设置需要提醒的话术以及次数。能不能叫醒看是否睡的沉。
